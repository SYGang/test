...
public class BoardListProcessor {
	...
	public BoardListProcessor(DaoBoard dao, String position, String currentPage, String word) {
		super();
		this.dao = dao;
	...
	public void getList() {
		...
		posts = dao.selectList(position, startIndex);
	}
	public void getList(String word) {
		int startIndex = (currentPage-1)*Board.LIST_AMOUNT;	//시작 인덱스 계산해서 넘기기
		posts = dao.selectList(position, startIndex,word);
	}
	/* 총 페이지 수 구하기 */
	public int getPageCount() {
		int totalPageCount = 0;
		int count = dao.selectPostCount(position);
	...
	public int getPageCount(String word) {
		int totalPageCount = 0;
		int count = dao.selectSearchPostCount(position, word);
		if(count % Board.LIST_AMOUNT == 0){		//case1. 나머지가 없이 딱 떨어지는 경우
			totalPageCount = count / Board.LIST_AMOUNT;
		}else{					//case2. 나머지가 있어서 짜투리 페이지가 필요한 경우
			totalPageCount = count / Board.LIST_AMOUNT + 1;
	...
	public String getHtmlPageList() {
		...
		if(hasPrev){
			if(word==null) {
				html=html+String.format("<a href='/board/list?position=%s&page=%d'>🐿️이전블럭가기🐿️</a>",position,prevPage);
			}else {
				html=html+String.format("<a href='/board/list?position=%s&page=%d&word=%s'>🐿️이전블럭가기🐿️</a>",position,prevPage,word);
			}
		}		
		...
		for(int i=blockStartNo;i<=blockEndNo;i++){	// 	<< 이렇게 바꿈
			if(word==null) {
				html = html + String.format("<a href='/board/list?position=%s&page=%d'>%d</a>&nbsp;&nbsp;",position,i,i);
			}else {
				html = html + String.format("<a href='/board/list?position=%s&page=%d&word=%s'>%d</a>&nbsp;&nbsp;",position,i,word,i);
			}
		}
		...
		if(hasNext){
			if(word==null) {
				html=html+String.format("<a href='/board/list?position=%s&page=%d'>🐿️다음블럭가기🐿️</a>",position,nextPage);
			}else {
				html=html+String.format("<a href='/board/list?position=%s&page=%d&word=%s'>🐿️다음블럭가기🐿️</a>",position,nextPage,word);
			}			
